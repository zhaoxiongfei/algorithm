...
const searchPaths = (beginWord, endWord, getOneDiff, rpaths) => {
  ...
  if (!paths[paths.length - 1].has(beginWord)) return [];
  return paths;
};
...
const getSolutions = (paths, getOneDiff) => {
  ...
  for (let i = 0; i < paths.length; i += 1) {
    const path = Array.from(paths[i]);
    if (path.length === 1) {
      solutions.forEach(x => x.push(path[0]));
    } else {
      solutions.forEach(x => {
...
// 从beginWord => endWord 以及 endWord => beginWord 做两次广度搜索，直到出现了对方即停止，这样能保证路径最短
// 之后逐层做两个集合的交集
