...
/**
  视频游戏“辐射4”中，任务“通向自由”要求玩家到达名为“Freedom Trail Ring”的金属表盘，
  并使用表盘拼写特定关键词才能开门。
...
const findRotateSteps = (ring, key) => {
  ...
  for (let i = 0; i < length; i += 1)
    map.set(ring[i], (map.get(ring[i]) || new Set()).add(i));

  // dp[i][j] 代表 key 的第 i 个字符，如果起始位置是 ring 的第j个字符至少需要多少步骤
  // dp[i][j] = dp[i - 1][x] +
  // Math.min(Math.abs(x - j), Math.abs(x + length - j), Math.abs(j + length - x));
  const dp = Array(n);
  for (let i = 0; i < n; i += 1) dp[i] = Array(length).fill(Infinity);

  for (const i of map.get(key[0]))
    dp[0][i] = 1 + Math.min(i, Math.abs(length - i));
  for (let i = 1; i < n; i += 1) {
    for (const j of map.get(key[i])) {
      let min = Infinity;
      for (const x of map.get(key[i - 1])) {
        const value =
          1 +
          dp[i - 1][x] +
          Math.min(
