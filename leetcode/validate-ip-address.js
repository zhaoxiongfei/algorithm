...
const validIPAddress = IP => {
  const base16 = /^[0-9abcdef]+$/;
  const base10 = /^[0-9]+$/;
  const ipv4 = IP.split(".");
  const ipv6 = IP.toLowerCase().split(":");
  if (ipv4.length === 1 && ipv6.length === 1) return "Neither";
  if (1 < ipv4.length && 1 < ipv6.length) return "Neither";

  if (ipv4.length === 4) {
    for (const p of ipv4) {
      if (!p.length) return "Neither";
      if (p.length > 1 && p[0] === "0") return "Neither";
      if (!base10.test(p)) return "Neither";
      if (+p > 255 || +p < 0) return "Neither";
    }
    return "IPv4";
  }
  if (ipv6.length === 8) {
    for (const p of ipv6) {
      if (!p.length || p.length > 4) return "Neither";
      if (!base16.test(p)) return "Neither";
    }
    return "IPv6";
  }
