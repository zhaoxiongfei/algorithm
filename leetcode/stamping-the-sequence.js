...
const movesToStamp = (stamp, target) => {
  ...
  let indexOf = -n;
  while ((indexOf = target.indexOf(stamp, indexOf + n)) !== -1) {
    ans.push(indexOf);
    for (let i = 0; i < n; i += 1) goal[indexOf + i] = "*";
    nonStarCount -= n;
  }

  if (nonStarCount === m) return [];

  while (0 < nonStarCount) {
    let nextLoop = false;
    for (let i = 0; i < m; i += 1) {
      let j = 0;
      let k = i;
      let matched = false;
      while (j < n && k < m && (goal[k] === stamp[j] || goal[k] === "*")) {
        if (stamp[j] === goal[k]) matched = true;
        j += 1;
        k += 1;
      }
      if (matched && j === n) {
...
console.log(movesToStamp("mda", "mdadddaaaa"));
console.log(movesToStamp("abca", "aabcaca"));
console.log(movesToStamp("lemk", "lleme"));
